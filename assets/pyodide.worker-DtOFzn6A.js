...
sys.stdout = _capture
sys.stderr = _capture
`),self.postMessage({type:"LOG",msg:">>> Pyodide ready. All execution is local (WASM)."}),t)}self.onmessage=async o=>{const{type:a,payload:r}=o.data;if(a==="INIT")try{await n(),self.postMessage({type:"READY"})}catch(e){self.postMessage({type:"ERROR",error:e.message})}else if(a==="SET_PORT")s=o.ports[0];else if(a==="SET_CANVAS")o.data.canvas;else if(a==="LOAD_EXCEL"){self.postMessage({type:"LOG",msg:">>> Converting Excel file via pandas to Arrow…"});try{t.FS.writeFile("/data.xlsx",new Uint8Array(r.buffer)),await t.runPythonAsync(`
import pandas as pd
...
`);const e=t.globals.get("_arrow_buffer"),l=new Uint8Array(e);t.globals.get("_arrow_buffer").destroy(),s&&s.postMessage({type:"ARROW_DATA",source:"python",tableName:"data",buffer:l},[l.buffer]),self.postMessage({type:"EXCEL_LOADED"})}catch(e){self.postMessage({type:"ERROR",error:e.message})}}else if(a==="RUN_CODE"){self.postMessage({type:"LOG",msg:">>> Running analysis…"});try{r.csvData&&t.FS.writeFile("/data.csv",new TextEncoder().encode(r.csvData)),await t.runPythonAsync(`${r.code}

_output = _capture.get_output()`);const e=t.globals.get("_output")||"";let l;try{await t.runPythonAsync(`
try:
    ...
    _plot_html = ''
`),l=t.globals.get("_plot_html")||void 0}catch{}try{await t.runPythonAsync(`
try:
    ...
    _out_arrow = None
`);const i=t.globals.get("_out_arrow");if(i&&s){const p=new Uint8Array(i);s.postMessage({type:"ARROW_DATA",source:"python",tableName:r.exportTableName||"python_data",buffer:p},[p.buffer]),i.destroy()}}catch{}self.postMessage({type:"CODE_RESULT",output:e||"Analysis complete.",plotHtml:l})}catch(e){self.postMessage({type:"ERROR",error:e.message})}}}})();
